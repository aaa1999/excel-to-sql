...
- **目录树与范围搜索**：左侧"大表（Excel 文件）▸ 子表"两级目录，勾选子表即搜索范围（可跨大表）；支持正向（勾选范围）/反向（未勾选范围）查找；**跨表查找会预先校验所选子表表头结构是否一致（列名+类型），不一致则提示差异并拒绝**；范围结果按子表汇总命中数并联动明细；拖动子表可更换所属大表（仅改目录归属，不动数据）
